...
def data_dir(tmp_path, monkeypatch):
    """Point DATA_DIR at tmp_path and set the container name, URL and token Armadillo would inject."""
    monkeypatch.setattr(data_mod, "DATA_DIR", tmp_path)
    monkeypatch.setattr(data_mod, "CONTAINER_NAME", "flower-client-1")
    monkeypatch.setattr(data_mod, "get_node_url", lambda: "https://armadillo.example.com")
    monkeypatch.setattr(data_mod, "get_node_token", lambda msg: "my-token")
    return tmp_path
...
def test_posts_to_armadillo(mock_request, data_dir):
    ...
    load_data(MSG, "myproject", "train.parquet")
...
def test_reads_and_deletes_file(mock_request, data_dir):
    ...
    assert load_data(MSG, "proj", "data/train") == b"raw file content"
